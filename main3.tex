...
When the simulated and experimental profiles are displayed on common axes, the difference in proton normalisation must be absorbed into a single scale factor. The simulation is scored per primary proton (units $\mathrm{proton^{-1}\,mm^{-1}\,sr^{-1}}$), whereas the experimental yield of equation~(\ref{eq:yield_exp}) is expressed per $10^9$ delivered protons. Each simulated profile is therefore multiplied by one global constant before being overlaid; in the figures shown this constant is $10^{11}$, i.e.\ the $10^9$-proton normalisation of equation~(\ref{eq:yield_exp}) together with an additional factor of $10^2$ that brings the simulated curves onto a vertical range comparable to the reproduced experimental panels. Being a uniform multiplicative factor applied identically to every depth point, this rescaling leaves the profile shape — and in particular the position and steepness of the distal fall-off that encode the proton range — completely unchanged; only the absolute height of the curves is affected.
