...
The 4.44~MeV ($^{12}$C) and 9.6~MeV ($^{16}$O) yield profiles are presented together in Figure~\ref{fig:44_96}, and the 6.13~MeV ($^{16}$O) profile is shown separately in Figure~\ref{fig:613}. It should be noted that the 9.6~MeV line, while accessible in the Geant4 simulation, was not measured in the Cracow experiment and is therefore presented here as a Geant4-only result without an experimental counterpart.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.75\linewidth]{images/c_44_96_90deg.png}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.75\linewidth]{images/c_6p13MeV_90deg.png}
    \caption{Geant4 prompt-gamma yield profile for the 6.13~MeV ($^{16}$O) line at 90$^\circ$ detection angle as a function of normalised depth $z/d_\mathrm{BP}$. Units are the same as in Figure~\ref{fig:44_96}.}
    \label{fig:613}
\end{figure}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.95\linewidth]{images/PG_comparison_130MeV_90deg.png}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.95\linewidth]{images/PG_comparison_130MeV_120deg.png}
...
The simulated $^{12}$C$_{4.44\to\mathrm{g.s.}}$ yield profile at 90$^\circ$ is shown alongside the corresponding experimental PMMA profile from~\cite{kelleter2017} in Figure~\ref{fig:comp_44}. Both profiles exhibit a pronounced distal fall-off at the position of the theoretical proton range in PMMA (indicated by the vertical red line in the experimental panel), confirming that the simulation correctly reproduces the correlation between the prompt-gamma drop and the Bragg peak position. The calculations reported by Kelleter~et~al.\ using literature cross sections reproduced the shape of the distal fall-off to within 15\% in scale and with a horizontal shift below 0.5~mm, consistent with the systematic uncertainty of the target thickness calibration.
...
\begin{figure}[h]
    \centering
    \subfloat[Geant4 (this work)]{\includegraphics[width=0.48\linewidth]{images/c_44_96_90deg.png}}
    \hfill
    \subfloat[Experiment, from \cite{kelleter2017}]{\includegraphics[width=0.48\linewidth]{images/figure3_1.png}}
    \caption{Prompt-gamma depth profiles at 90$^\circ$ detection angle for the $^{12}$C$_{4.44\to\mathrm{g.s.}}$ line. \textit{Left}: Geant4 simulation (this work) for PMMA at 130.87~MeV; the yield $Y = N_\gamma/(FOV\cdot\Delta\Omega_\mathrm{sim})$ per proton is plotted vs.\ normalised depth $z/d_\mathrm{BP}$. The 4.44~MeV line is shown in blue, the 9.6~MeV line in green. \textit{Right}: experimental measurement from Kelleter~et~al.~\cite{kelleter2017} for PMMA at 130.87~MeV; the vertical red line marks the theoretical proton range. Both panels show a clear distal fall-off correlated with the Bragg peak position.}
    \label{fig:comp_44}
\end{figure}

\begin{figure}[h]
    \centering
    \subfloat[Geant4 (this work)]{\includegraphics[width=0.48\linewidth]{images/c_6p13MeV_90deg.png}}
    \hfill
    \subfloat[Experiment, from \cite{kelleter2017}]{\includegraphics[width=0.48\linewidth]{images/figure3_2.png}}
    \caption{Prompt-gamma depth profiles for the $^{16}$O$_{6.13\to\mathrm{g.s.}}$ line at 90$^\circ$. \textit{Left}: Geant4 simulation (this work). \textit{Right}: experimental measurement from Kelleter~et~al.~\cite{kelleter2017}. A quantitative comparison is not made because Geant4 is known to inadequately reproduce the cross sections for this transition.}
    \label{fig:comp_613}
\end{figure}
