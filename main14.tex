...
\textbf{Legendre coefficients near the Bragg peak.}
The anisotropy coefficients $a_2$ and $a_4$ of equation~(\ref{eq:legendre}) were extracted from the solid-angle-normalised differential yield $\mathrm{d}N/\mathrm{d}\Omega(\theta)$ at the three depth points closest to the Bragg peak ($d_\mathrm{BP} = 107$~mm for the 130.87~MeV beam and $d_\mathrm{BP} \approx 33$~mm for the 70.54~MeV beam), where the line statistics are highest and the fit is most stable, and the extraction was repeated for the two analysed lines and for each of the three physics lists of Section~\ref{sec:physlists}. Figures~\ref{fig:homog_130} and~\ref{fig:homog_70} show the leading coefficient $a_2$ as a function of $z - d_\mathrm{BP}$ for the two beam energies, with the three physics lists overlaid and the dashed line marking the isotropic expectation $a_2 = 0$; the fitted coefficients at the Bragg peak are collected in Table~\ref{tab:legendre}.
% TODO: fill in the numeric values below and in Table~\ref{tab:legendre} from
%       angular_homogeneity_summary.csv produced by analyze_angular_homogeneity.C.
...
\begin{figure}[h]
    ...
    \caption{Leading Legendre anisotropy coefficient $a_2$ of the prompt-gamma angular distribution as a function of $z - d_\mathrm{BP}$ at the three depths closest to the Bragg peak, for the 130.87~MeV beam: (a) the 4.44~MeV $^{12}$C line and (b) the 9.6~MeV $^{16}$O line. The three curves compare the \texttt{FTFP\_BERT\_HP}, \texttt{QBBC} and \texttt{QGSP\_BIC\_HP\_EMZ} physics lists; the dashed line marks the isotropic value $a_2 = 0$.}
    \label{fig:homog_130}
\end{figure}
...
\begin{table}[h]
\centering
\caption{Fitted even-order Legendre coefficients $a_2$ and $a_4$ (equation~(\ref{eq:legendre})) of the prompt-gamma angular distribution at the Bragg peak ($z - d_\mathrm{BP} \approx 0$), for both beam energies, both analysed lines and the three physics lists. Uncertainties are the fit uncertainties.}
\label{tab:legendre}
\begin{tabular}{lllcc}
\toprule
