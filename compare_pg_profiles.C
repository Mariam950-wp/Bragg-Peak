...
///
/// The number of physics-list subdirectories is NOT fixed: every immediate
/// subdirectory of `motherDir` whose name contains `energyTag` and holds a
/// PG_analysis.root is picked up automatically and overlaid on the same
/// pad, however many there are (2, 5, 10, ...).
///
/// Expected layout (subdirectories for several beam energies and any number
/// of physics lists can coexist in the same motherDir; `energyTag` selects
...
///     prompt_gamma_spectra_130MeV_FTFP_BERT_HP/PG_analysis.root
///     prompt_gamma_spectra_130MeV_QGSP_BIC_EMY/PG_analysis.root
...
// Style palette cycles (via modulo) once the number of physics-list runs
// exceeds its length, so any number of runs is supported.
static const Int_t kColors[] = {
    kBlue + 1, kRed + 1, kGreen + 2, kMagenta + 1,
    kOrange + 7, kCyan + 2, kAzure + 1, kViolet + 1,
    kSpring + 5, kPink + 6, kTeal + 2, kYellow + 2
};
static const Int_t kMarkers[] = { 20, 21, 22, 23, 33, 34, 47, 43, 24, 25, 26, 32 };
...
//     entry — there is no limit on how many are picked up. Pass an empty
//     energyTag to match every subdirectory.
// ================================================================
...
//     the current pad, for a given gamma line and detector angle. The
//     legend's column count, height and text size all scale with the
//     number of runs, so any number of physics lists stays readable.
// ================================================================
                     ...
                     const std::vector<std::string>& labels)
{
    ...
    const Int_t nRuns = (Int_t)graphs.size();
    const Int_t nCols = nRuns <= 3 ? 1 : (nRuns <= 8 ? 2 : 3);
    const Int_t nRows = (Int_t)std::ceil((double)nRuns / nCols);
    const Double_t textSize = nRows > 6 ? 0.020 : (nRows > 3 ? 0.024 : 0.028);
    const Double_t rowHeight = textSize + 0.006;

    const Double_t legX2 = 0.89;
    const Double_t legX1 = nCols == 1 ? 0.65 : (nCols == 2 ? 0.52 : 0.40);
    const Double_t legY2 = 0.89;
    const Double_t legY1 = std::max(0.30, legY2 - rowHeight * nRows);
